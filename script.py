...
with Connection(master_ip, user='ubuntu', connect_kwargs={'key_filename': 'mysql.pem'}) as c:
    ...
    c.get('/tmp/powerapi_master_1.txt', local='results/powerapi_master_1.txt')
    c.get('/tmp/powerapi_master_2.txt', local='results/powerapi_master_2.txt')

with Connection(slave_1_ip, user='ubuntu', connect_kwargs={'key_filename': 'mysql.pem'}) as c:
    c.get('/tmp/powerapi_slave1_1.txt', local='results/powerapi_slave1_1.txt')
    c.get('/tmp/powerapi_slave1_2.txt', local='results/powerapi_slave1_2.txt')

with Connection(slave_2_ip, user='ubuntu', connect_kwargs={'key_filename': 'mysql.pem'}) as c:
    c.get('/tmp/powerapi_slave2_1.txt', local='results/powerapi_slave2_1.txt')
    c.get('/tmp/powerapi_slave2_2.txt', local='results/powerapi_slave2_2.txt')

with Connection(proxy_ip, user='ubuntu', connect_kwargs={'key_filename': 'mysql.pem'}) as c:
    c.get('/tmp/powerapi_proxy_1.txt', local='results/powerapi_proxy_1.txt')
    c.get('/tmp/powerapi_proxy_2.txt', local='results/powerapi_proxy_2.txt')

with Connection(gatekeeper_ip, user='ubuntu', connect_kwargs={'key_filename': 'mysql.pem'}) as c:
    c.get('/tmp/powerapi_gatekeeper_1.txt', local='results/powerapi_gatekeeper_1.txt')
    c.get('/tmp/powerapi_gatekeeper_2.txt', local='results/powerapi_gatekeeper_2.txt')
